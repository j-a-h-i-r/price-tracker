...
export async function getAllDeals(filter: DealsQuery): Promise<CurrentDeal[]> {
    ...
    let priceSortString = '';
    if (sortby === 'value') {
        priceSortString = 'mpw.max_price_last_days - lp.price desc';
    } else if (sortby === 'percentage') {
        priceSortString = '((mpw.max_price_last_days - lp.price) / mpw.max_price_last_days) desc';
    }
    const { rows } = await knex.raw(`
        WITH MaxPriceLastDays AS (
        ...
        WHERE
            lp.price < mpw.max_price_last_days
